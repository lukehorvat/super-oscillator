...
export default class Synthesizer extends THREE.Group {
  ...
  createBottomPanel() {
    ...
    this.bottomPanel.material = new THREE.MeshPhysicalMaterial({
      color: '#3a3a3a',
      emissive: '#1a1a1a',
      reflectivity: 0.1,
      metalness: 0.1,
  ...
  createLeftPanel() {
    ...
    mesh1.geometry = new THREE.ShapeGeometry(
      new THREE.Shape([
    ...
    mesh2.geometry = new THREE.ShapeGeometry(
      new THREE.Shape([
        new THREE.Vector2(0, 0),
  ...
  createScreen() {
    ...
    let height =
      (this.options.height -
        this.backPanel.bbox.height -
        this.bottomPanel.bbox.height) /
      3;
    let depth = this.backPanel.bbox.depth * 0.6;

    this.screen = new THREE.Mesh();
    this.screen.material = new THREE.MeshPhysicalMaterial({
      color: '#000000',
      emissive: '#161616',
      reflectivity: 0.2,
      metalness: 0.8,
  ...
  createScreenText() {
    let oscillatorName =
      Object.keys(oscillators)[this.oscillatorIndex].toUpperCase();
    ...
    this.screenText.geometry = new THREE.TextGeometry(oscillatorName, {
      font: Synthesizer.font,
      size: this.screen.bbox.depth * 0.4,
      height: 1,
    });
    this.screenText.bbox.centerX = this.screen.bbox.centerX;
    this.screenText.bbox.minY = this.screen.bbox.maxY;
    this.screenText.bbox.minZ =
      this.screen.bbox.centerZ + this.screenText.bbox.height / 2;
  ...
  createLeftButton() {
    ...
    this.leftButton.material = new THREE.MeshPhysicalMaterial({
      color: '#111111',
      emissive: '#1a1a1a',
      reflectivity: 0.1,
      metalness: 0.7,
    ...
    mesh4.geometry = new THREE.ShapeGeometry(
      new THREE.Shape([
        new THREE.Vector2(0, 0),
        new THREE.Vector2(depth, 0),
    ...
    this.leftButton.bbox.maxX =
      this.screen.bbox.minX -
      (this.backPanel.bbox.depth - this.screen.bbox.depth) / 2;
  ...
  createRightButton() {
    ...
    this.rightButton.bbox.minX =
      this.screen.bbox.maxX +
      (this.backPanel.bbox.depth - this.screen.bbox.depth) / 2;
  ...
  createKey(note, index, notes) {
    ...
    let keyGap = 2;
    let keyWidth =
      (this.backPanel.bbox.width -
        this.leftPanel.bbox.width -
        this.rightPanel.bbox.width -
        (notes.length + 1) * keyGap) /
      notes.length;
    let keyBlackHeight = this.backPanel.bbox.height / 2;
    let keyWhiteHeight = keyBlackHeight / 1.75;
    let keyWhiteDepth =
      this.bottomPanel.bbox.depth - this.backPanel.bbox.depth - keyGap;
    ...
    if ([1, 3, 6, 8, 10].includes(semitone)) {
      key.material = new THREE.MeshPhysicalMaterial({
        color: '#333333',
        emissive: '#000000',
        reflectivity: 0.1,
        metalness: 0.1,
      });
      key.geometry = new THREE.BoxGeometry(
        keyWidth,
        keyBlackHeight,
        keyBlackDepth
      );
    ...
    key.position.x =
      this.leftPanel.bbox.maxX +
      keyGap +
      keyWidth / 2 +
      (keyWidth + keyGap) * index;
  ...
  addInputListener(renderer, camera) {
    ...
    renderer.domElement.addEventListener('mousedown', (event) => {
      ...
      this.clickedObject = clickableObjects.find((object) =>
        camera.isObjectAtCoord({
          object,
          x: event.clientX,
          y: event.clientY,
          renderer,
    ...
    renderer.domElement.addEventListener('mousemove', (event) => {
      let object = clickableObjects.find((object) =>
        camera.isObjectAtCoord({
          object,
          x: event.clientX,
          y: event.clientY,
      ...
      if (
        ...
        this.keys.includes(this.clickedObject) &&
        this.keys.includes(object) &&
        this.isMouseDown
      ) {
        this.onMouseUp();
        this.clickedObject = object;
  ...
  onMouseDown() {
    ...
    if ([this.leftButton, this.rightButton].includes(this.clickedObject)) {
      ...
      this.oscillatorIndex = wrapIndex(
        this.oscillatorIndex +
          (this.clickedObject === this.rightButton ? 1 : -1),
        Object.entries(oscillators).length
      );
      this.remove(this.screenText);
    } else if (this.keys.includes(this.clickedObject)) {
      this.clickedObject.bbox.minY -= this.clickedObject.userData.pressHeight;
      this.oscillator = Object.values(oscillators)[this.oscillatorIndex](
        this.audioContext
      );
